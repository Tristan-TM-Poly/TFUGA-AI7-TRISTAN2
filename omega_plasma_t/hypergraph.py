...
class PlasmaHypergraph:
    ...
    def canonical_dict(self)->dict:
        return {"nodes":[asdict(self.nodes[k]) for k in sorted(self.nodes)],"hyperedges":[asdict(self.edges[k]) for k in sorted(self.edges)]}

    def fingerprint(self)->str:
        raw=json.dumps(self.canonical_dict(),sort_keys=True,separators=(",",":"),ensure_ascii=False).encode()
        return hashlib.sha256(raw).hexdigest()

    def audit(self)->dict:
        referenced={x for e in self.edges.values() for x in (*e.sources,*e.targets)}
        isolated=sorted(set(self.nodes)-referenced)
        kinds={}
