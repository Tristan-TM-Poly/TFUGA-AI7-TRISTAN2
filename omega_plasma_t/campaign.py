...
class CampaignPolicy:
    ...
    growth_factor:float=1.7
    pressure_soft:float=0.75
    pressure_hard:float=0.95
...
class CampaignGenerator:
    ...
    def emit_jsonl(self,path:Path,resume_after:int=0,work_budget:int|None=None)->dict:
        """No permanent total cap. work_budget is an execution budget, not an architecture ceiling."""
        path.parent.mkdir(parents=True,exist_ok=True); count=0; skipped=0; start=time.time()
        ...
        with path.open(mode,encoding="utf-8") as f:
            for index,item in enumerate(self.combinations()):
                ...
                if self.policy.checkpoint_every and count%self.policy.checkpoint_every==0:
                    cp={"last_index":index,"written":count,"path":str(path),"timestamp":time.time()}
                    path.with_suffix(".checkpoint.json").write_text(json.dumps(cp,indent=2),encoding="utf-8")
