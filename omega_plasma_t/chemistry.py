...
RateLaw=Callable[[dict[str,float],float],float]
...
class ReactionNetwork:
    ...
    def species(self)->tuple[str,...]:
        return tuple(sorted({x for r in self.reactions for x in (*r.reactants,*r.products)}))
    ...
    def euler(self,c0:dict[str,float],dt_s:float,steps:int,clip:bool=True)->list[dict[str,float]]:
        ...
        for _ in range(steps):
            dc=self.derivative(c)
            c={s:(max(0.0,c[s]+dt_s*dc[s]) if clip else c[s]+dt_s*dc[s]) for s in c}
            hist.append(dict(c))
        return hist

    def stoichiometric_matrix(self)->dict[str,dict[str,float]]:
        return {s:{r.reaction_id:r.net(s) for r in self.reactions} for s in self.species}
