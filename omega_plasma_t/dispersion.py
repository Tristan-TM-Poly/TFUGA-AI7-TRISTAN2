...
from .constants import ELEMENTARY_CHARGE, EPSILON_0, ELECTRON_MASS
...
def langmuir_bohm_gross(k_rad_m:float,n_e_m3:float,t_e_ev:float)->DispersionPoint:
    wp=electron_plasma_angular_frequency(n_e_m3); vth2=2*t_e_ev*ELEMENTARY_CHARGE/ELECTRON_MASS
    return DispersionPoint(k_rad_m,complex(sqrt(max(0.0,wp*wp+1.5*k_rad_m*k_rad_m*vth2)),0),"langmuir_bohm_gross",("Maxwellian electrons","immobile ions","weak damping"))
...
def ion_acoustic(k_rad_m:float,t_e_ev:float,ion_mass_kg:float,lambda_d_m:float=0.0)->DispersionPoint:
    ...
    return DispersionPoint(k_rad_m,complex(k_rad_m*cs/sqrt(1+(k_rad_m*lambda_d_m)**2),0),"ion_acoustic",("Te >> Ti","quasi-neutral bulk","fluid ions"))
