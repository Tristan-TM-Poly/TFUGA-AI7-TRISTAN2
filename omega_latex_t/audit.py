...
def _safe_subaudit(out: list[AuditFinding], name: str, function: Callable[[DocumentIR], Mapping[str, Any]], doc: DocumentIR) -> Mapping[str, Any]:
    try:
        report = function(doc)
        if not isinstance(report, Mapping): raise TypeError(f"{name} returned non-object report")
        return report
    except Exception as exc:
        out.append(AuditFinding(f"{name.upper()}_SUBAUDIT_EXCEPTION", "error", f"{name} sub-audit failed safely: {exc.__class__.__name__}: {exc}"))
        return {}
...
def audit_document(doc: DocumentIR) -> AuditReport:
    ...
    try:
        bibliography_findings = validate_bibliography(doc)
    except Exception as exc:
        out.append(AuditFinding("BIBLIOGRAPHY_SUBAUDIT_EXCEPTION", "error", f"bibliography sub-audit failed safely: {exc.__class__.__name__}: {exc}")); bibliography_findings = ()
    for finding in bibliography_findings: out.append(AuditFinding(finding["code"], finding["severity"], finding["message"]))
    ...
    for node in doc.nodes:
        ...
        if node.kind == NodeKind.FIGURE and node.figure_ir:
            try: figure_findings = validate_figure_ir(node.figure_ir)
            except Exception as exc: out.append(AuditFinding("FIGURE_IR_SUBAUDIT_EXCEPTION", "error", f"FigureIR validation failed safely: {exc.__class__.__name__}: {exc}", node.id)); figure_findings = ()
            for finding in figure_findings: out.append(AuditFinding(finding["code"], finding["severity"], finding["message"], node.id))
    ...
    for key in sorted(doc.results):
        ...
        try: result_findings = validate_result(doc.results[key])
        except Exception as exc: out.append(AuditFinding("RESULT_SUBAUDIT_EXCEPTION", "error", f"result validation failed safely: {exc.__class__.__name__}: {exc}", key)); result_findings = ()
        for finding in result_findings: out.append(AuditFinding(finding["code"], finding["severity"], finding["message"], key))

    receipt_report = _safe_subaudit(out, "verifier_receipt", verifier_receipt_report, doc)
    for item in receipt_report.get("entries", ()):
    ...
    fragment_report = _safe_subaudit(out, "source_fragment", source_fragment_report, doc)
    for item in fragment_report.get("entries", ()): _append_external_findings(out, item.get("findings", ()))

    covariance_report = _safe_subaudit(out, "covariance", covariance_ledger, doc)
    for item in covariance_report.get("entries", ()): _append_external_findings(out, item.get("findings", ()))

    lineage = _safe_subaudit(out, "proof_lineage", proof_lineage, doc); _append_external_findings(out, lineage.get("findings", ()))

    metadata_report = _safe_subaudit(out, "metadata_receipt", metadata_receipt_report, doc)
    for item in metadata_report.get("entries", ()):
        if item.get("valid") is False: out.append(AuditFinding("METADATA_RECEIPT_INVALID", "error", "; ".join(item.get("reasons", []))))
        elif item.get("verified") is False: out.append(AuditFinding("METADATA_RECEIPT_UNVERIFIED", "warning", "; ".join(item.get("reasons", []))))

    figure_backend_report = _safe_subaudit(out, "figure_backend", figure_backend_manifest, doc)
    for item in figure_backend_report.get("figures", ()): _append_external_findings(out, item.get("findings", ()), node_id=str(item.get("node_id", "")))
