...
def theorem_bundle(doc: DocumentIR, theorem_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "formal_projection": {
            ...
            "statement_supplied": bool(formal_statement),
            "verified_flag_supplied": formal_verified,
            "status": "verified-metadata" if formal_statement and formal_verified else "stub-or-unverified",
        },
        "numerical_test_contract": numerical_contract if isinstance(numerical_contract, dict) else {},
        "boundary": (
            "narrative proof, formal proof metadata and numerical evidence remain distinct; "
            "generated stubs are never proof"
        ),
    }
...
def write_theorem_bundle(doc: DocumentIR, theorem_id: str, output_dir: str | Path) -> dict[str, Path]:
    ...
    proof_graph.write_text(
        json.dumps(
            {
                "theorem_id": theorem_id,
                "dependencies": list(theorem.dependencies),
                "proof_nodes": payload["proof_nodes"],
                "sources": list(theorem.sources),
            },
            ensure_ascii=False,
            indent=2,
    ...
    if system.lower() == "lean" and formal_statement:
        formal_path.write_text(
            (
                "-- AUTO-GENERATED PROOF OBLIGATION STUB; `sorry` means UNPROVEN HERE.\n"
    ...
    numerical_path.write_text(
        json.dumps(
            payload["numerical_test_contract"],
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
        )
    ...
    paths = {
        "theorem": theorem_json,
        "latex": theorem_tex,
        "proof_graph": proof_graph,
        "formal_stub": formal_path,
        "numerical_contract": numerical_path,
    }
    manifest = {
        "schema_version": "1.0.0",
        "theorem_id": theorem_id,
