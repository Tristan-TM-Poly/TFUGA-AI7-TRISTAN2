...
from omega_infra_qc_t import GraphMLExporter, JsonExporter, build_demo_artifacts
from omega_infra_qc_t.cli import main


def test_demo_builder_returns_report_bundle_and_graphml():
    ...
    assert "<graphml" in artifacts.graphml
    assert "redacted_asset" in artifacts.graphml
...
def test_graphml_exporter_is_public_safe():
    artifacts = build_demo_artifacts()
    assert "Demo Water Facility" not in artifacts.graphml
    assert "redacted_asset" in artifacts.graphml
    assert "Generalized review-only demo facility" not in artifacts.graphml
    assert GraphMLExporter is not None


def test_cli_demo_writes_report_bundle_and_graphml(tmp_path):
    ...
    assert (out_dir / "infra_qc_demo.graphml").exists()
...
def test_cli_individual_outputs(tmp_path):
    ...
    assert main(["graphml", "--out", str(graphml_path)]) == 0
