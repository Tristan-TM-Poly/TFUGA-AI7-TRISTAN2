...
Move QC-DIGEST from a source-kernel scaffold toward a local pilot workflow that can produce a reviewed offline report from Tristan-owned public metadata, Tristan-owned research directions, invention map, or explicitly selected open metadata without committing generated outputs by default.
