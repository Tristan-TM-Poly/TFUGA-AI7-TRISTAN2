# Ω-GITHUB-CUMULATIVE-MEMORY-T∞ — R0.1
...
R0.1 intentionally reuses instead of duplicating:
...
The new code is therefore restricted to the previously missing residual:
...
Stores four different evidence classes without collapsing them:

```text
formal Capability observations
PR historical records
asset candidates (changed files / atlas candidates)
typed graph edges
```
...
Changed files and exact source should then be fetched only for the highest-ranked historical candidates before implementation.
...
1. runs the GitHub-memory OAK court;
2. snapshots all PR metadata using `GITHUB_TOKEN` with read permissions;
3. ingests the Capability OS registry;
4. builds a context packet for a deterministic smoke request;
5. uploads the memory/context as CI artifacts.
...
R0.1 is intentionally a deterministic retrieval/architecture baseline. A future semantic ranker may improve recall, but it must preserve the same provenance classes and OAK separation rather than converting similarity into truth.

## Next bounded generations

- **R0.2 Symbol/AST Genome** — index public classes/functions/imports for candidate PR heads without loading all source into LLMT context.
