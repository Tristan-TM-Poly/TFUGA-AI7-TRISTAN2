...
def run_oakbench() -> dict:
    ...
    return {"bench_version": "0.2.0", "deterministic": True, "capabilities": ["resource_graph", "material_passport", "route_optimization", "small_coupled_flow_oracle", "uncertainty_sweep", "industrial_symbiosis", "urban_mine"], "plan": {"total_score": round(plan.total_score, 6), "recovered_value": round(plan.recovered_value, 6), "retained_mass_kg": round(plan.retained_mass_kg, 6), "dry_run_only": plan.dry_run_only, "evaluations": [{"component_id": item.component_id, "mode": item.mode.value, "score": round(item.score, 6), "recovered_value": round(item.recovered_value, 6), "total_cost": round(item.total_cost, 6), "retained_mass_kg": round(item.retained_mass_kg, 6), "dry_run_only": item.dry_run_only, "warnings": list(item.warnings)} for item in plan.evaluations]}, "component_entropy": {candidate.component.component_id: round(material_entropy(candidate.component), 6) for candidate in candidates}, "oak": asdict(report), "limits": ["synthetic benchmark only", "small-instance exact coupled solver only", "no lifecycle-assessment certification", "no physical processing authorization"]}
