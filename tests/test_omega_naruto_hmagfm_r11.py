...
from omega_naruto_hmagfm.gates import GateDecision, GatePolicy, evaluate_publication
from omega_naruto_hmagfm.genjutsu import (
    GenjutsuCode,
    audit_proposal,
    has_blocking_finding,
)
from omega_naruto_hmagfm.integration import to_claim_packet, to_mminus_registry
...
def test_custom_gate_policy_can_raise_the_maturity_floor() -> None:
    ...
    report = evaluate_publication(item, policy=policy, human_review_completed=True)
    assert report.decision is GateDecision.BLOCK
