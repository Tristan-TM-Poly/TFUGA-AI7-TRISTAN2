# Ω-CPUFMT — CPU fractal mycélien

**Identifiant :** `omega-cpufmt`  
**Version :** `R0.2-public`  
**Maturité :** `architecture`  
**Preuve :** `simulation requise`

## Résumé

Architecture de calcul modulaire explorant topologies NoC, caches multi-échelles, redondance et contrôle thermo-énergétique.

## Statut épistémique

Aucune supériorité PPA, thermique ou manufacturière n’est revendiquée avant synthèse, simulation et comparaison.

Une architecture n’est pas une preuve; un prototype n’est pas un produit validé.

## Domaines

- matériel
- RISC-V
- systèmes

## Profil OAK provisoire

| Dimension | Signal |
|---|---:|
| verite | 0.47 |
| utilite | 0.86 |
| testabilite | 0.77 |
| simplicite | 0.52 |
| valeur | 0.76 |
| protection | 0.62 |

Les scores servent à naviguer; ils ne sont ni probabilités de vérité ni certifications.

## Claims publics

### claim-omega-cpufmt-01 — Portée publique et objet testable

- Type: `scope`
- Statut: `candidate`
- Énoncé: Ω-CPUFMT propose un cadre structuré pour architecture de calcul modulaire explorant topologies NoC, caches multi-échelles, redondance et contrôle thermo-énergétique.
- Limite: La portée publique ne démontre ni supériorité générale, ni causalité, ni validité hors du domaine déclaré.
- Prochain test: Transformer l’objet en entrée, sortie, baseline, métrique et seuil reproductibles.

### claim-omega-cpufmt-02 — Limite OAK obligatoire

- Type: `limit`
- Statut: `guardrail`
- Énoncé: La promotion de Ω-CPUFMT reste bloquée tant que hypothèses, unités, résidus, risques et contre-exemples ne sont pas exposés.
- Limite: Un garde-fou éditorial réduit la surpromesse mais ne valide pas le modèle sous-jacent.
- Prochain test: Auditer page, dépôt, tests et sources pour détecter une divergence réelle.

### claim-omega-cpufmt-03 — Prochaine falsification ou réduction

- Type: `test-plan`
- Statut: `planned`
- Énoncé: La prochaine progression de Ω-CPUFMT doit produire une comparaison mesurée plutôt qu’une nouvelle extension nominale.
- Limite: Le plan peut échouer, être sous-dimensionné ou mesurer un proxy inadéquat.
- Prochain test: Construire un simulateur de topologies et comparer latence, énergie, surface et tolérance aux défauts.

## Relations sortantes

- `crystallized_by` → `omega-tristan-self-os` — Traverse capture, canon, OAK, prototype, IP et valeur.
- `documented_by` → `omega-doc-t` — Claims, versions, limites et résidus restent documentés.
- `mapped_by` → `omega-atlas-t` — Coordonnées, provenance, statut, risques et routes restent navigables.
- `uncertainty_guard` → `omega-unc2-t` — Incertitudes, désaccords et domaines de validité restent explicites.
- `published_through` → `omega-web-tristan-t` — La couche publique expose seulement le résumé validé par les quatre gates.
- `shares_domain` → `omega-mems-cpu-t` — Domaine partagé: matériel
- `same_family` → `omega-laser-mems-cpu-t` — Famille partagée: hardware

## Relations entrantes


## Risques

- surpromesse
- preuve insuffisante
- sécurité physique
- fabricabilité

## Artefacts attendus

- fiche publique
- rapport OAK
- prochaine expérience
- spécification
- schéma
- protocole de test

## Prochaine action

Construire un simulateur de topologies et comparer latence, énergie, surface et tolérance aux défauts.

## Provenance

- Source: `docs/theory_cards/OMEGA_CPUFMT.md`
- Mise à jour: `2026-08-02`

## Gates

```text
PUBLIC = OAKGate AND IPGate AND PrivacyGate AND SecurityGate
```

Aucune action externe automatique ou divulgation d’IP n’est autorisée par cette fiche.
