# Ω-FCRYST-T — Théorie des cristaux fractals

**Identifiant :** `omega-fcryst-t`  
**Version :** `R0.2-public`  
**Maturité :** `architecture`  
**Preuve :** `simulation et expérience`

## Résumé

Étudie ordre multi-échelle, défauts, pores, interfaces, diffraction et propriétés de matériaux architecturés.

## Statut épistémique

Il faut distinguer cristal classique, quasi-cristal, motif artificiel, modèle et matériau expérimental.

Une architecture n’est pas une preuve; un prototype n’est pas un produit validé.

## Domaines

- cristallographie
- matériaux
- fractales

## Profil OAK provisoire

| Dimension | Signal |
|---|---:|
| verite | 0.43 |
| utilite | 0.76 |
| testabilite | 0.73 |
| simplicite | 0.48 |
| valeur | 0.72 |
| protection | 0.63 |

Les scores servent à naviguer; ils ne sont ni probabilités de vérité ni certifications.

## Claims publics

### claim-omega-fcryst-t-01 — Portée publique et objet testable

- Type: `scope`
- Statut: `candidate`
- Énoncé: Ω-FCRYST-T propose un cadre structuré pour étudie ordre multi-échelle, défauts, pores, interfaces, diffraction et propriétés de matériaux architecturés.
- Limite: La portée publique ne démontre ni supériorité générale, ni causalité, ni validité hors du domaine déclaré.
- Prochain test: Transformer l’objet en entrée, sortie, baseline, métrique et seuil reproductibles.

### claim-omega-fcryst-t-02 — Limite OAK obligatoire

- Type: `limit`
- Statut: `guardrail`
- Énoncé: La promotion de Ω-FCRYST-T reste bloquée tant que hypothèses, unités, résidus, risques et contre-exemples ne sont pas exposés.
- Limite: Un garde-fou éditorial réduit la surpromesse mais ne valide pas le modèle sous-jacent.
- Prochain test: Auditer page, dépôt, tests et sources pour détecter une divergence réelle.

### claim-omega-fcryst-t-03 — Prochaine falsification ou réduction

- Type: `test-plan`
- Statut: `planned`
- Énoncé: La prochaine progression de Ω-FCRYST-T doit produire une comparaison mesurée plutôt qu’une nouvelle extension nominale.
- Limite: Le plan peut échouer, être sous-dimensionné ou mesurer un proxy inadéquat.
- Prochain test: Simuler une structure architecturée et prédire une signature de diffraction mesurable.

## Relations sortantes

- `crystallized_by` → `omega-tristan-self-os` — Traverse capture, canon, OAK, prototype, IP et valeur.
- `documented_by` → `omega-doc-t` — Claims, versions, limites et résidus restent documentés.
- `mapped_by` → `omega-atlas-t` — Coordonnées, provenance, statut, risques et routes restent navigables.
- `uncertainty_guard` → `omega-unc2-t` — Incertitudes, désaccords et domaines de validité restent explicites.
- `published_through` → `omega-web-tristan-t` — La couche publique expose seulement le résumé validé par les quatre gates.

## Relations entrantes

- `shares_domain` ← `omega-bat-t` — Domaine partagé: matériaux
- `shares_domain` ← `omega-3dp-t` — Domaine partagé: matériaux

## Risques

- surpromesse
- preuve insuffisante

## Artefacts attendus

- fiche publique
- rapport OAK
- prochaine expérience
- spécification
- schéma
- protocole de test

## Prochaine action

Simuler une structure architecturée et prédire une signature de diffraction mesurable.

## Provenance

- Source: `docs/theory_cards/OMEGA_FCRYST_T.md`
- Mise à jour: `2026-08-02`

## Gates

```text
PUBLIC = OAKGate AND IPGate AND PrivacyGate AND SecurityGate
```

Aucune action externe automatique ou divulgation d’IP n’est autorisée par cette fiche.
