...
from .types import MathType, ScalarSystem, StructureKind, TypeSystemError, UnitDimension
...
class OperatorProperty:
    ...
    def __post_init__(self) -> None:
        if self.status not in {"declared", "tested", "proved", "refuted", "unknown"}:
...
class OperatorExpr:
    ...
    @classmethod
    def symbol(cls, name: str, math_type: MathType, *, properties: Iterable[str] = ()) -> "OperatorExpr":
        return cls(
            kind=OperatorKind.SYMBOL,
            name=name,
            declared_type=math_type,
            properties=tuple(OperatorProperty(value) for value in sorted(set(properties))),
        )

    @classmethod
    def identity(cls, math_type: MathType, *, name: str = "I") -> "OperatorExpr":
        _require_square_operator(math_type)
        return cls(OperatorKind.IDENTITY, name=name, declared_type=math_type, properties=(OperatorProperty("identity", "proved"),))

    @classmethod
    def zero(cls, math_type: MathType, *, name: str = "0") -> "OperatorExpr":
        return cls(OperatorKind.ZERO, name=name, declared_type=math_type, properties=(OperatorProperty("zero", "proved"),))
    ...
    def matrix(
        ...
        *,
        ...
        return cls(OperatorKind.MATRIX_LITERAL, name=name, declared_type=math_type, matrix_value=rows)

    @classmethod
    def unary(cls, kind: OperatorKind, operand: "OperatorExpr", **kwargs: Any) -> "OperatorExpr":
        return cls(kind=kind, operands=(operand,), **kwargs)

    @classmethod
    def binary(cls, kind: OperatorKind, left: "OperatorExpr", right: "OperatorExpr", **kwargs: Any) -> "OperatorExpr":
        return cls(kind=kind, operands=(left, right), **kwargs)

    @classmethod
    def nary(cls, kind: OperatorKind, operands: Iterable["OperatorExpr"], **kwargs: Any) -> "OperatorExpr":
    ...
    def infer_type(self) -> MathType:
        if self.kind in {OperatorKind.SYMBOL, OperatorKind.IDENTITY, OperatorKind.ZERO, OperatorKind.MATRIX_LITERAL}:
        ...
        if self.kind in {OperatorKind.EXPONENTIAL, OperatorKind.LOGARITHM}:
            ...
            if not operand_type.units.is_dimensionless:
                raise TypeSystemError(f"{self.kind.value} requires a dimensionless operator")
            return operand_type
        ...
        if self.kind == OperatorKind.DIRECT_SUM:
            ...
            for value in types[1:]:
                from .types import common_scalar_system
                scalar = common_scalar_system(scalar, value.scalar_system)
            codomain = _dimension_sum([value.shape.matrix_codomain().value for value in types])
            domain = _dimension_sum([value.shape.matrix_domain().value for value in types])
            units = types[0].units
            if any(value.units != units for value in types[1:]):
                raise TypeSystemError("direct-sum blocks must share operator units")
            return MathType.linear_operator(codomain, domain, scalar_system=scalar, units=units)
        if self.kind == OperatorKind.KRONECKER_SUM:
            ...
            if left.units != right.units:
                raise TypeSystemError("Kronecker-sum operands must share units")
            return left.tensor_result(right)
        ...
        if self.declared_type is not None:
            return self.declared_type
        raise TypeSystemError(f"type inference is not implemented for {self.kind.value}")

    def depth(self) -> int:
        return 1 if not self.operands else 1 + max(operand.depth() for operand in self.operands)
    ...
    def has_property(self, name: str, *, minimum_status: str | None = None) -> bool:
        ranking = {"unknown": 0, "declared": 1, "tested": 2, "proved": 3, "refuted": -1}
        for prop in self.properties:
            if prop.name == name:
                return minimum_status is None or ranking[prop.status] >= ranking[minimum_status]
        return False
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "name": self.name,
            "declared_type": None if self.declared_type is None else self.declared_type.to_dict(),
            "scalar_value": _json_scalar(self.scalar_value),
            "matrix_value": [[_json_scalar(value) for value in row] for row in self.matrix_value],
    ...
    def simplify(self, *, max_passes: int = 32) -> "OperatorExpr":
        ...
        raise OperatorError("simplification did not converge within max_passes")
    ...
    def scale(self, scalar: complex | float | int) -> "OperatorExpr":
        return OperatorExpr.unary(OperatorKind.SCALAR_MULTIPLY, self, scalar_value=scalar)
    ...
    def tensor(self, other: "OperatorExpr") -> "OperatorExpr":
        return OperatorExpr.binary(OperatorKind.TENSOR_PRODUCT, self, other)

    def commutator(self, other: "OperatorExpr") -> "OperatorExpr":
        return OperatorExpr.binary(OperatorKind.COMMUTATOR, self, other)

    def anticommutator(self, other: "OperatorExpr") -> "OperatorExpr":
        return OperatorExpr.binary(OperatorKind.ANTICOMMUTATOR, self, other)
...
def _validate_arity(kind: OperatorKind, arity: int) -> None:
    zero = {OperatorKind.SYMBOL, OperatorKind.IDENTITY, OperatorKind.ZERO, OperatorKind.MATRIX_LITERAL}
    unary = {
    ...
    nary = {OperatorKind.SUM, OperatorKind.COMPOSE, OperatorKind.TENSOR_PRODUCT, OperatorKind.DIRECT_SUM}
...
def _require_square_operator(math_type: MathType) -> None:
    if math_type.structure != StructureKind.LINEAR_OPERATOR:
        raise TypeSystemError("operation requires a finite linear operator")
    if not math_type.shape.matrix_codomain().compatible_with(math_type.shape.matrix_domain()):
        raise TypeSystemError("operation requires a square operator")
    if math_type.domain_id is not None and math_type.codomain_id is not None and math_type.domain_id != math_type.codomain_id:
        raise TypeSystemError("operation requires matching named domain and codomain")
...
def _flatten(kind: OperatorKind, operands: Iterable[OperatorExpr]) -> tuple[OperatorExpr, ...]:
...
def _simplify_once(expression: OperatorExpr) -> OperatorExpr:
    if not expression.operands:
        return expression
    operands = tuple(operand.simplify(max_passes=8) for operand in expression.operands)
    current = OperatorExpr(
    ...
    if current.kind == OperatorKind.ADJOINT:
        ...
        if operand.kind == OperatorKind.ZERO:
            return OperatorExpr.zero(operand.infer_type().adjoint_result())
        if operand.kind == OperatorKind.COMPOSE:
            return OperatorExpr.nary(
                OperatorKind.COMPOSE,
                tuple(value.adjoint() for value in reversed(operand.operands)),
            )
        if operand.kind in {OperatorKind.SUM, OperatorKind.DIFFERENCE}:
            return OperatorExpr(
                kind=operand.kind,
                operands=tuple(value.adjoint() for value in operand.operands),
            )
    ...
    if current.kind == OperatorKind.COMPOSE:
        ...
        flat = [value for value in flat if value.kind != OperatorKind.IDENTITY]
        if not flat:
    ...
    if current.kind == OperatorKind.SUM:
        ...
        flat = [value for value in flat if value.kind != OperatorKind.ZERO]
        if not flat:
            return OperatorExpr.zero(current.infer_type())
        if len(flat) == 1:
            return flat[0]
    ...
    if current.kind == OperatorKind.SCALAR_MULTIPLY:
        ...
        if scalar == 1:
            return operand
        if operand.kind == OperatorKind.SCALAR_MULTIPLY:
            return operand.operands[0].scale(scalar * operand.scalar_value)

    if current.kind == OperatorKind.COMMUTATOR:
        left, right = operands
        if left == right:
            return OperatorExpr.zero(left.infer_type())
        if left.kind == OperatorKind.IDENTITY or right.kind == OperatorKind.IDENTITY:
            return OperatorExpr.zero(left.infer_type())

    if current.kind == OperatorKind.ANTICOMMUTATOR:
        left, right = operands
        if left.kind == OperatorKind.ZERO or right.kind == OperatorKind.ZERO:
            return OperatorExpr.zero(left.infer_type())
    ...
    if current.kind == OperatorKind.TENSOR_PRODUCT:
        ...
        if len(flat) == 1:
            return flat[0]
        return OperatorExpr.nary(OperatorKind.TENSOR_PRODUCT, flat)

    return current


def operator_expression_from_dict(payload: Mapping[str, Any]) -> OperatorExpr:
    from .types import math_type_from_dict

    scalar = payload.get("scalar_value")
    if isinstance(scalar, Mapping) and "real" in scalar and "imag" in scalar:
        scalar = complex(float(scalar["real"]), float(scalar["imag"]))
    matrix = tuple(
        tuple(
            complex(value["real"], value["imag"])
            if isinstance(value, Mapping) and "real" in value and "imag" in value
            else value
            for value in row
        )
    ...
    return OperatorExpr(
        ...
        operands=tuple(operator_expression_from_dict(value) for value in payload.get("operands", [])),
        name=str(payload.get("name", "")),
        declared_type=None if payload.get("declared_type") is None else math_type_from_dict(payload["declared_type"]),
        scalar_value=scalar,
        matrix_value=matrix,
        exponent=payload.get("exponent"),
        attributes=tuple(sorted(payload.get("attributes", {}).items())),
        properties=tuple(
            OperatorProperty(
                name=str(value["name"]),
