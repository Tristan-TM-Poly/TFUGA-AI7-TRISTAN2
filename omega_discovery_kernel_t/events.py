"""Canonical event language for Ω-DISCOVERY-KERNEL-T∞.

The event model is append-only, deterministic, and intentionally conservative:
a recorded event is evidence about a workflow transition, not proof that the
scientific interpretation in its payload is true.
"""
...
class DiscoveryEvent:
    ...
    def validate(self) -> list[str]:
        ...
        if self.event_type == "ExperimentSpec" and not self.reversible and not self.human_approval:
            issues.append(f"{self.event_id}: irreversible experiment requires human approval")
